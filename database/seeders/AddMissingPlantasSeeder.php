...
class AddMissingPlantasSeeder extends Seeder
{
    /**
     * Seeder to create the non-canonical plants from `plantas.txt` using the curator-assigned Crowley numbers.
     * Default: DRY RUN (no DB writes). Set env `ADD_PLANTAS_DRY_RUN=false` to apply.
     * To overwrite existing plant data set `ADD_PLANTAS_OVERWRITE=true`.
     */
    public function run(): void
    {
        $dryRun = filter_var(env('ADD_PLANTAS_DRY_RUN', 'true'), FILTER_VALIDATE_BOOLEAN);
        ...
        // List of non-canonical plants to add (normalized keys)
        $toAdd = [
            'aguacate',
            'anturio blanco',
            'bambu',
        ...
        foreach ($parts as $part) {
            ...
            if ($plant) {
                // update only missing fields or if overwrite
